...
public abstract class AbstractDAO<T> {
    ...
    public List<T> getAllByFields(Class<T> cls, String...args) {
        List<T> res = new ArrayList<>();
        String sql = Arrays.stream(args).collect(Collectors.joining(", "));
        sql = "SELECT " + sql.trim().substring(0,sql.length()) + " FROM ";
        try {
            try (Statement st = conn.createStatement()) {
